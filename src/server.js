...
function requireSetupAuth(req, res, next) {
  if (!SETUP_PASSWORD) {
    return res
      .status(500)
      .type("text/plain")
      .send("SETUP_PASSWORD is not set. Set it in Northflank Variables before using /setup.");
  }
...
app.get("/setup/api/debug", requireSetupAuth, async (_req, res) => {
  ...
  res.json({
    wrapper: {
      ...
      northflankCommit: process.env.NORTHFLANK_GIT_COMMIT_SHA || null,
    },
    openclaw: {
  ...
  try {
    ...
    if (!isUnderDir(STATE_DIR, dataRoot) || !isUnderDir(WORKSPACE_DIR, dataRoot)) {
      return res
        .status(400)
        .type("text/plain")
        .send("Import is only supported when OPENCLAW_STATE_DIR and OPENCLAW_WORKSPACE_DIR are under /data (Northflank volume).\n");
    }
